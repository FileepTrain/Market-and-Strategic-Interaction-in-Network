...
from market_clearing_alg import run_interactive_clearing, run_clearing_until_equilibrium
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--plot",
        action="store_true",
        help="Plot the final (or per-round, with --interactive) bipartite graph."
    )
...
def main():
    ...
    if args.interactive:
        run_interactive_clearing(
            ...
            ask_each_round=True,
            max_rounds=1000,
            plot=args.plot,   # draws per round only if --plot is set
        )
        if args.plot:
            plt.show()
        return
    ...
    if not args.plot and not args.interactive:
        # Terminal-only mode: unified dumper prints the final state
        run_clearing_until_equilibrium(G, verbose=True)
        return
