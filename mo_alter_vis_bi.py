...
def draw_prices(ax, pos, sellers, G=None, prices_override=None, dx=-0.22):
    """Draw price text to the LEFT of each seller."""
    for i, s in enumerate(sellers):
        if prices_override is not None:
            price = fmt_num(prices_override[i])
        else:
    ...
    for i, b in enumerate(buyers):
        ...
        if isinstance(vec, (list, tuple)) and vec:
            ...
            for j, v in enumerate(vec):
                ...
                # move right a little
                #offset += len(s_val) * 0.01
                offset += 0.05
                

                # draw comma except after last value
                #if j < len(vec) - 1:
                #    offset += 0.05
                #    ax.text(x + dx + offset, y, ", ",
